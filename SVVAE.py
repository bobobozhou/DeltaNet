...
class SVVAE:
    ...
    def _build_graph_(self):
        ...
        with tf.variable_scope('level1_DC') as scope:  # LEVEL 1 for Decoder
            ...
            # Up conv for final output (generate 128x128x64 with x1 - Binary Segmentation Volume)
            in_filters = 16
            out_filters = 1
            ...
            biases = self._bias_variable('biases2_l1', [out_filters])
            y_recon = tf.nn.bias_add(conv2_l1, biases)
        ...
        recon_loss = tf.reduce_mean(-tf.reduce_sum(y * tf.log(eps + y_recon) + (1 - y) * tf.log(1 - y_recon + eps), axis=1))
